...
export const useBooking = (settings: ReservationSettings): UseBookingResult => {
  ...
  useEffect(() => {
    const fetchAvailability = async (): Promise<void> => {
      ...
      try {
        ...
        if (!response.ok) {
          setStatus(
            createStatus({
              error: data.error ?? "Unable to load availability.",
            }),
          );
    ...
    fetchAvailability();
  }, [state.date, state.partySize, settings.timezone, settings.lead_time_min]);
  ...
  const setPartySize = (size: number): void =>
    dispatch({ type: "setPartySize", payload: size });
  const setSelected = (slot: string | null): void =>
    dispatch({ type: "setSelected", payload: slot });
  const setGuestName = (name: string): void =>
    dispatch({ type: "setGuestName", payload: name });
  const setGuestPhone = (phone: string): void =>
    dispatch({ type: "setGuestPhone", payload: phone });
  ...
  const book = async (): Promise<void> => {
    ...
    try {
      ...
      setStatus(
        createStatus({
          success: `Reservation confirmed for ${state.selected}`,
        }),
      );
      reset();
      ...
      setStatus(
        createStatus({
          error: "Unable to complete reservation. Please try again.",
        }),
      );
      reset();

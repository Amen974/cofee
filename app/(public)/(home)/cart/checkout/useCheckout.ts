...
export const useCheckout = () => {
  ...
  const [submitState, submitDispatch] = useReducer(
    submitReducer,
    INITIAL_SUBMIT,
  );
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error)
        return console.error("[useCheckout] fetchSettings failed", error);
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        submitDispatch({
          type: "ERROR",
          payload: "Unable to access your location",
        });
        reset();
      },
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    if (!validateName(formState.name))
      return submitDispatch({
        type: "ERROR",
        payload: "Please enter a valid name (2-100 characters)",
      });
    ...
    if (
      !hasCoordinates &&
      hasAddress &&
      !validateAddress(deliveryState.address)
    )
      return submitDispatch({
        type: "ERROR",
        payload: "Please enter a valid address (5-200 characters)",
      });
    ...
    if (!hasCoordinates && !hasAddress)
      return submitDispatch({
        type: "ERROR",
        payload:
          "Please provide a delivery address or use your current location",
      });
    ...
    if (error) {
      ...
      return submitDispatch({
        type: "ERROR",
        payload: "Failed to place order. Please try again later.",
      });
    }
